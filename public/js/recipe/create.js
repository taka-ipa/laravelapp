window.onload = function() {
  ...
  steps.addEventListener('click', function(evt) {
    if (evt.target.classList.contains('step-delete') || evt.target.closest('.step-delete') ) { // クリックされた要素がstep-deleteクラスを持つか、その親要素がstep-deleteクラスを持つ場合
      evt.target.closest('.step').remove(); // クリックされた要素の親要素のstepクラスを持つ要素を削除
      var items = steps.querySelectorAll('.step');
      items.forEach(function(item, index) {
        item.querySelector('.step-number').innerHTML = '手順'+ (index + 1);
    });
  ...
  addStep.addEventListener('click', function() {  
    var stepCount = steps.querySelectorAll('.step').length; // 手順の数を取得
    ...
    `;
    steps.appendChild(step); // stepsの子要素として追加
  });
  // 材料の追加
